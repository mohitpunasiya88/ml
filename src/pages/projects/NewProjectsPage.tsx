...
const NewProjectsPage = () => {
  ...
  const [selectedProjects, setSelectedProjects] = useState<string[]>([]);
    ...
    const fetchProjects = async () => {
      try {
        ...
        const response = await api.get('/projects?status=New');
  ...
  if (e.target.checked) {
    setSelectedProjects(filteredProjects.map(project => project.id)); // Fix: use project.id
  } else {
    setSelectedProjects([]);
  }
};
  ...
  const handleExport = () => {
    ...
    exportToExcel(projectsToExport, 'New_Projects');
  ...
  const handleSendToCEO = async () => {
    ...
    try {
      ...
      await api.post('/projects/update', { projectIds: selectedProjects, status:"Sent to CEO" });
      
      // Mock update for demonstration
      await new Promise(resolve => setTimeout(resolve, 1000));
      fetchProjects()
      toast.success(`${selectedProjects.length} projects sent to CEO`);
